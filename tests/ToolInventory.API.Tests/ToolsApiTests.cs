...
public class ToolsApiTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [Fact]
    public async Task CreateTool_WithoutBarcode_GeneratesGuidBarcode()
    {
        var createToolResponse = await _client.PostAsJsonAsync("/api/tools", new
        {
            name = "Generated Barcode Tool",
            description = "Barcode should be generated by the system"
        });
        var createdTool = await createToolResponse.Content.ReadFromJsonAsync<ToolResponse>();

        Assert.Equal(HttpStatusCode.Created, createToolResponse.StatusCode);
        Assert.False(string.IsNullOrWhiteSpace(createdTool!.Barcode));
        Assert.Matches("^[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}$", createdTool.Barcode);
    }

    [Fact]
    public async Task UpdateTool_DoesNotAllowChangingBarcode()
    {
        var createToolResponse = await _client.PostAsJsonAsync("/api/tools", new
        {
            name = "Immutable Barcode Tool",
            description = "Created for barcode immutability test"
        });
        var createdTool = await createToolResponse.Content.ReadFromJsonAsync<ToolResponse>();
        var originalBarcode = createdTool!.Barcode;

        var updateResponse = await _client.PutAsJsonAsync($"/api/tools/{createdTool.Id}", new
        {
            name = "Immutable Barcode Tool",
            description = "Updated description",
            barcode = "MANUAL-BARCODE-SHOULD-BE-IGNORED",
            status = "Available"
        });

        var updatedToolResponse = await _client.GetAsync($"/api/tools/{createdTool.Id}");
        var updatedTool = await updatedToolResponse.Content.ReadFromJsonAsync<ToolResponse>();

        Assert.Equal(HttpStatusCode.NoContent, updateResponse.StatusCode);
        Assert.Equal(HttpStatusCode.OK, updatedToolResponse.StatusCode);
        Assert.Equal(originalBarcode, updatedTool!.Barcode);
    }
